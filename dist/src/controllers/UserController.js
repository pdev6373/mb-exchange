...
const CryptoPriceController_1 = require("./CryptoPriceController");
let UserController = class UserController {
    ...
    async getTransactions(req) {
        ...
        const transactions = await Transaction_1.TransactionModel.find({
            ...
            .sort({ createdAt: -1 })
            .lean();
        const total = await Transaction_1.TransactionModel.countDocuments({
            userId: req.user._id,
        });
    ...
    async makeTransaction(data, req) {
        ...
        const networks = assetExist.networks;
        const networkExist = networks.find((network) => network._id?.toString() === networkId);
        if (!networkExist)
            throw new customErrors_1.NotFoundError('Network not found');
        if (networkExist.address !== address)
        ...
        await User_1.UserModel.updateOne({ _id: req.user?._id }, { $inc: { pendingTransactions: 1, totalTransactions: 1 } });
        return (0, responseWrapper_1.successResponse)('Transaction in process');
    }
    async payReward(req) {
        const user = req.user;
    ...
    async getCryptoHistoricalData(symbol, timeframe, limit) {
        const data = await (0, CryptoPriceController_1.fetchHistoricalData)(symbol, timeframe, limit ? parseInt(limit) : 100);
        return (0, responseWrapper_1.successResponse)('Historical data retrieved successfully', data);
    }
};
...
__decorate([
    (0, tsoa_1.Get)('/crypto/historical'),
    ...
    __metadata("design:returntype", Promise)
], UserController.prototype, "getCryptoHistoricalData", null);
